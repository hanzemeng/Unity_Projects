...
namespace EmeraldAI
{
    ...
    public class EmeraldAISystem : MonoBehaviour
    {
        ...
        //Custom: Ability Attacks
        public int AbilityAttackIndex = 0;
        public bool isMeleeAbility = false;
        public bool isOffensiveAbility = false;
        public bool isSupportAbility = false;
        public bool isSummoningAbility = false;

        public int numberMeleeAttackAnimations = 0;
        public int numberMeleeRunAttacksAnimations = 0;
        public int numberOffensiveAbilitiesAnimations = 0;
        public int numberSupportAbilitiesAnimations = 0;
        public int numberSummoningAbilitiesAnimations = 0;

        public bool isUsingCustomAbility = false;

        public bool abilityOneOnCooldown = false;
        public bool abilityTwoOnCooldown = false;
        public bool abilityThreeOnCooldown = false;

        //Melee Attacks
        public int MeleeAttackIndex = 0;
        public enum MeleeAttackPickTypeEnum { Odds, Order, Random, Ability };
        ...
        public enum MeleeRunAttackPickTypeEnum { Odds, Order, Random, Ability };
        ...
        public enum OffensiveAbilityPickTypeEnum { Odds, Order, Random, Ability };
        ...
        public enum SupportAbilityPickTypeEnum { Odds, Order, Random, Ability };
        ...
        public enum SummoningAbilityPickTypeEnum { Odds, Order, Random, Ability };
        ...
        public IEnumerator InstantSwitchCurrentWeaponType(string WeaponTypeName)
        {
            IsSwitchingWeapons = true;
            yield return new WaitForSeconds(0.05f);
            EmeraldEventsManagerComponent.CancelAttackAnimation();
        ...
        public void Wander ()
        {
            ...
            {
                if (m_NavMeshAgent.remainingDistance <= m_NavMeshAgent.stoppingDistance && !m_NavMeshAgent.pathPending && m_NavMeshAgent.velocity.magnitude != 0)
                {
        ...
        /// </summary>
        void CreateEmeraldProjectile(bool isLast)
        {
        ...
        /// Added function for special attacks to avoid activation of default EAI behavior (1 = skip EAI)
        /// </summary>
        // public void EmeraldAttackEvent(int isSpecial)
        public void EmeraldAttackEvent(AnimationEvent animEvent)
        {
            bool isLast = animEvent.floatParameter > 0 ? false : true;
            if (animEvent.intParameter == 1)
            {
                ...
                if (WeaponTypeRef == WeaponType.Melee)
                {
                    SendEmeraldDamage(isLast);
                }
                else if (WeaponTypeRef == WeaponType.Ranged)
                {
                    CreateEmeraldProjectile(isLast);
                }
        ...
        /// </summary>
        public void SendEmeraldDamage(bool isLast)
        {
            AIAnimator.ResetTrigger("Hit");
                    ...
                    || WeaponTypeRef == WeaponType.Ranged && !SwitchWeaponTypeTriggered)
                {
                    ...
                    if (AdjustedAngle <= MaxDamageAngle || WeaponTypeRef == WeaponType.Ranged)
                    {
                        if (TargetTypeRef == TargetType.Player)
                        {
                            ...
                            {
                                PlayerDamageComponent.SendPlayerDamage(CurrentDamageAmount, this.transform, GetComponent<EmeraldAISystem>(), CriticalHit, isLast);
                            }
        ...
        public void Damage (int DamageAmount, TargetType? TypeOfTarget = null, Transform AttackerTransform = null, int RagdollForce = 100, bool CriticalHit = false)
        {
            ...
            DamageReceived = (int) (defenseModifier * DamageAmount);
            ...
            if (AttackerTransform != CurrentFollowTarget && BehaviorRef == CurrentBehavior.Companion && !IsDead || BehaviorRef != CurrentBehavior.Companion && BehaviorRef != CurrentBehavior.Pet && !IsDead)
            {
                ...
                {
                    float BlockDamage = Mathf.Abs((DamageAmount * ((MitigationAmount - 1) * 0.01f)) - DamageAmount) * defenseModifier;
        ...
        public void GetDamageAmount()
        {
            if (RandomizeDamageRef == YesOrNo.Yes)
            {
                ...
                {
                    CurrentDamageAmount = (int)(damageModifier * Random.Range(MeleeAttacks[MeleeAttacksListIndex].MinDamage, MeleeAttacks[MeleeAttacksListIndex].MaxDamage));
                }
                else if (CurrentMeleeAttackType == CurrentMeleeAttackTypes.RunAttack)
                {
                    CurrentDamageAmount = (int)(damageModifier * Random.Range(MeleeRunAttacks[MeleeRunAttacksListIndex].MinDamage, MeleeRunAttacks[MeleeRunAttacksListIndex].MaxDamage));
                }
            }
            else if (RandomizeDamageRef == YesOrNo.No)
            {
                if (CurrentMeleeAttackType == CurrentMeleeAttackTypes.StationaryAttack)
                {
                    CurrentDamageAmount = (int)(damageModifier * MeleeAttacks[MeleeAttacksListIndex].MinDamage);
                }
                else if (CurrentMeleeAttackType == CurrentMeleeAttackTypes.RunAttack)
                {
                    CurrentDamageAmount = (int)(damageModifier * MeleeRunAttacks[MeleeRunAttacksListIndex].MinDamage);
                }
            }
            ...
            if (UseCriticalHits == YesOrNo.Yes && WeaponTypeRef == WeaponType.Melee)
            {
                ...
                if (CriticalHit)
                {
                    CurrentDamageAmount = (int)(damageModifier * Mathf.RoundToInt(CurrentDamageAmount * CritMultiplier));
                }
            }
        }
        ...
        public void ModSpeedStats(System.Func<float, float> modifier) {
            WalkSpeed = modifier(WalkSpeed);
            RunSpeed = modifier(RunSpeed);
            Attack1AnimationSpeed = modifier(Attack1AnimationSpeed);
            Attack2AnimationSpeed = modifier(Attack2AnimationSpeed);
            Attack3AnimationSpeed = modifier(Attack3AnimationSpeed);
            Attack4AnimationSpeed = modifier(Attack4AnimationSpeed);
            Attack5AnimationSpeed = modifier(Attack5AnimationSpeed);
            Attack6AnimationSpeed = modifier(Attack6AnimationSpeed);
            RangedAttack1AnimationSpeed = modifier(RangedAttack1AnimationSpeed);
            RangedAttack2AnimationSpeed = modifier(RangedAttack2AnimationSpeed);
            RangedAttack3AnimationSpeed = modifier(RangedAttack3AnimationSpeed);
            RangedAttack4AnimationSpeed = modifier(RangedAttack4AnimationSpeed);
            RangedAttack5AnimationSpeed = modifier(RangedAttack5AnimationSpeed);
            RangedAttack6AnimationSpeed = modifier(RangedAttack6AnimationSpeed);
        }
    }
}
